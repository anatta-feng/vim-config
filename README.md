...
# 安装 YCM
在`./init.spacevim`中的`spacevim_layers`数组中添加`ycmd`，然后运行`install.py`会自动下载安装YCM。

然后在 `~/.vim/plugged/YouCompleteMe` 中执行 `./install.py --clang-completer --go-completer --java-completer`

其他语言支持请参照 [YCM 说明文档](https://github.com/Valloric/YouCompleteMe#mac-os-x)

需要先行安装好依赖，C Family Language 支持需要 clang，其他语言需要相应的环境
